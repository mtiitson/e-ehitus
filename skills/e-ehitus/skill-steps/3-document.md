...
```bash
ehr-api GET "/api/geoinfo/v1/getgeoobjectsbyaddress?address=Tartu+mnt+1&epsg=wgs84&rgo=false" \
...
ehr-api POST /api/document/v1/document/$DOC_TYPE \
...
DOC_NR=$(ehr-api POST /api/document/v1/document/11271 \
...
ehr-api DELETE /api/document/v1/document/2611271/04701 | jq .
...
ehr-api GET /api/document/v1/document/building/base/11271/EHR_CODE | jq .
```
...
```bash
ehr-api GET /api/document/v1/prosecutingAuthority/BUILDING_ID | jq .
```

## Step 3b — Fetch baseline building data

```bash
ehr-api GET "/api/building/v2/buildingData?ehr_code=EHR_CODE" | jq .
```
