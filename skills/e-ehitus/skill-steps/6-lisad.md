...
ehr-api GET /api/classifier/v1/classifier/faty/11271 | jq '[.[] | {id: .fatyId, name: .fatyName}]'
...
ehr-api PUT /api/document/v1/document/DOC_NR/file/FILE_ID \
  '{"title": "...", "publisher": "...", "notes": "...", "docNumber": "..."}' | jq .

# List current attachments
ehr-api GET /api/document/v1/document/DOC_NR/files | jq .

# Delete attachment
ehr-api DELETE /api/file-upload-api/v1/DOC_NR/file/FILE_ID | jq .
```
...
```bash
DOC_ID=$(ehr-api GET /api/document/v1/document/DOC_NR | jq '.documentId')

# 1 — create project container
PROJECT_ID=$(ehr-api POST /api/building-project-file/v1/application/$DOC_ID/project \
  '{}' | jq -r '.id')
...
# 3 — set file metadata
ehr-api PUT /api/building-project-file/v1/application/$DOC_ID/project/$PROJECT_ID/file/$FILE_ID \
  '{
...
ehr-api GET /api/building-project-file/v1/application/$DOC_ID/project/current | jq .

# Full classifier list (grupp, projektiOsa, projektiStaadium codes)
ehr-api GET /api/building-project-file/v1/config | jq .
```
...
DOC_VERSION=$(ehr-api GET /api/document/v1/document/DOC_NR | jq -r '.documentVersion // "1"')
ehr-api GET /api/document/v1/document/DOC_NR > /tmp/doc.json
ehr-api POST /api/document/v1/document/DOC_NR/$DOC_VERSION/stateFee \
  @/tmp/doc.json | jq .
```
